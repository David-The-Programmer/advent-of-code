    ...
    try {
      ...
      for(Pair<String, List<Integer>> a : antennas) {
        ...
        for(Pair<String, List<Integer>> b : antennas) {
          ...
          List<Integer> antiNodeA = vectorPosOnLine(antennasInLine.get(0), dirVector(antennasInLine.get(1), antennasInLine.get(0)), -1);
          List<Integer> antiNodeB = vectorPosOnLine(antennasInLine.get(0), dirVector(antennasInLine.get(1), antennasInLine.get(0)), 2);
          List<List<Integer>> antiNodes = List.of(antiNodeA, antiNodeB).stream().filter(x -> !isOutOfMap(grid, x)).toList();
